...
int main() {
  ...
  if(commands.size() != 0 ) {
    ...
    if(strcmp(c.filename(), "exit") == 0) exit(0);  

    pid_t pid = fork();
    int status;
    if (pid == 0 ) {
      has_error = execvp(c.filename(), c.argv());
      if(has_error == -1) {
        cout << "Programa não encontrado, tente de novo bjs, leia a documentação também, muito útil :)" << endl; 
      }
    } else {
      wait(&status);  
    }
